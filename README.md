...
Для сборки из исходников нужны [Node.js 18+](https://nodejs.org/), [Rust stable](https://rustup.rs/) и системные зависимости Tauri (на Windows — [C++ Build Tools](https://visualstudio.microsoft.com/visual-cpp-build-tools/) и [WebView2](https://developer.microsoft.com/en-us/microsoft-edge/webview2/)). Подробности по каждой ОС — в [docs/build-and-deploy.md](./docs/build-and-deploy.md).
...
Готовые установщики появляются в `src-tauri/target/release/bundle/` (Windows: `.exe` NSIS и `.msi` WiX).
...
Техническая документация — в каталоге `docs/` ([сборка и развёртывание](./docs/build-and-deploy.md), [архитектура](./docs/architecture.md)).
